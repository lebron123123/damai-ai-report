...
全流程AI选品分析:**数据收集 → 核心因子分析 → 分析报告 → 上架建议**。

- `/chat` — **默认首页**,对话式引导:选"该卖什么"还是"该不该上架",AI一步步问必要信息(不是一次性甩给你一个大表单),问完自动调用下面两条流水线,报告直接嵌在对话里
- `/recommend` —「我应该上架卖什么」:候选池召回 → 批量因子打分 → AI选品委员会横向对比 → 推荐排名(不想聊天、想直接填表的话,侧边栏"直接填表"里能进)
- `/evaluate` —「这个产品怎么样」:单品数据 → 因子分析 → 看多/看空AI辩论 → 决策委员会给出 上架/观察/不上架
- `/factors` — 因子分析独立页,只算因子打分,不跑LLM辩论/决策,查得更快
- `/history` — 我的记录,`/recommend`、`/evaluate`(不管是聊天触发的还是直接填表触发的)每次生成的报告都自动存下来,刷新不丢,点进去能看完整报告原样重现
...
src/lib/chat/       聊天引导流程的问题定义(flow.ts)。故意做成固定的引导式问卷而不是
                     自由对话+工具调用的agent——两个意图各自问1-2个固定问题,问完直接
                     调用现成的recommend/evaluate流水线,简单、可控、好调试。

src/lib/history/    「我的记录」持久化(store.ts),用 Node 内置的 node:sqlite(免原生编译,
                     所以 Dockerfile 用的是 node:24 而不是 node:20)。evaluate/recommend
                     的API路由跑完报告后会自动落库,不需要前端额外调用保存接口。
```
...
## 关于「我的记录」持久化的真实限制(重要,不是bug)

`src/lib/history/store.ts` 写的是容器本地磁盘上的一个 SQLite 文件(`.data/history.sqlite`,
和 ML 快照一样被 gitignore 了)。**本地开发、或者部署到有持久化卷的宿主机上完全没问题**,
但如果部署在腾讯云开发「云托管」这种默认无状态的容器上,**每次重新部署/重启容器,这个文件
会被清空**——这是容器化部署的通病,不是这次实现漏了什么。

真正的生产级修复是换成腾讯云开发自带的数据库(文档型/SQL型,你的环境里已经开通了,不用
额外开户),`HistoryStore` 这层接口已经抽象好了,到时候只需要重写 `store.ts` 这一个文件,
API路由和页面都不用动。现在先用 SQLite 把功能跑通,数据库切换留到你决定要不要长期部署时再做。
...
- 目前没有鉴权/多用户,「我的记录」是全局共享的,不区分是谁生成的报告
- 聊天引导流程目前是固定问卷,不支持追问("如果预算再高一点呢"这种多轮微调需要重新走一遍问答)
